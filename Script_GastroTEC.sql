...
----- Mas gustado por dia
SELECT Platillo.Nombre,
       max(Contador) AS Maximo
  FROM (
           SELECT idPlatillo AS id,
                  count(Puntuacion) AS Contador,
                  Fecha
             FROM PlatilloRating
            WHERE Puntuacion == 1
            GROUP BY idPlatillo
       ...
       INNER JOIN
       Platillo ON Platillo.idPlatillo == id
 WHERE Fecha == '2018-09-02';

-- Menos gustado por dia
...
SELECT Platillo.Nombre,
  ...
  FROM (
       ...
       INNER JOIN
       Platillo ON Platillo.idPlatillo == id
 WHERE Fecha BETWEEN '2018-01-15' AND '2018-06-25';


-- Menos gustado por semmestre
SELECT Platillo.Nombre,
       min(Contador) AS Minimo
  FROM (
           SELECT idPlatillo AS id,
...
---- Horario más frecuentado por los estudiantes--------
SELECT HoraComida,
       max(Horarios_mas_visitados) AS Hora_mas_frecuentada
  FROM (
           SELECT HoraComida,
                  count(HoraComida) AS Horarios_mas_visitados
             FROM Menu
            GROUP BY HoraComida
       );
...
------Carrera de estudiantes que más frecuenta cada restaurante--------------
SELECT max(Cantidad) AS CantidadEstudiantes,
       Carrera,
       Restaurante
  FROM (
           SELECT Count(CodigoCarrera) AS Cantidad,
                  Carrera,
                  Restaurante
             FROM (
                      SELECT Estudiante.idCarrera AS CodigoCarrera,
                             idEstudiante,
                  ...
                  INNER JOIN
                  (
                      SELECT idEstudiante AS ID,
                             Restaurante.Nombre AS Restaurante
                        FROM RestauranteRaiting
                             INNER JOIN
                             Restaurante ON RestauranteRaiting.idRestaurante = Restaurante.idRestaurante
                  )
                  ON idEstudiante = ID
...
-----------Estudiante más participativo de la aplicación--------

SELECT idEnPlatillo AS idEstudiante,
       max(Participacion) AS Participaciones,
       Nombre1 AS Nombre,
       Apellido1 AS Apellido
  FROM (
           SELECT idEnPlatillo,
                  idEnRestaurante,
                  count(Nombre1) AS Participacion,
                  Nombre1,
                  Apellido1
             FROM (
                      SELECT PlatilloRating.idEstudiante AS idEnPlatillo,
                             Estudiante.Nombre AS Nombre1,
                             Estudiante.Primer_Apellido AS Apellido1
                        FROM PlatilloRating
                             INNER JOIN
                             Estudiante ON PlatilloRating.idEstudiante = Estudiante.idEstudiante
                  )
                  INNER JOIN
                  (
                      SELECT RestauranteRaiting.idEstudiante AS idEnRestaurante,
                             Estudiante.Nombre,
                             Estudiante.Primer_Apellido
                        FROM RestauranteRaiting
                             INNER JOIN
                             Estudiante ON RestauranteRaiting.idEstudiante = Estudiante.idEstudiante
                  )
                  ON idEnPlatillo = idEnRestaurante
            WHERE idEnPlatillo = idEnRestaurante
            GROUP BY Nombre1
       );
;

---------- Rating para cada restaurante----------------------------------------------------------------
SELECT Restaurante.Nombre,
       ( (Votos_Favor - Votos_contra) / Votos_Totales) AS Rating
  FROM (
           SELECT idRestaurante AS ID,
                  sum(Puntuación) AS Votos_Favor,
                  (count(Puntuación) - sum(Puntuación) ) AS Votos_contra,
                  count(Puntuación) AS Votos_totales
             FROM RestauranteRaiting
            GROUP BY idRestaurante
       )
       INNER JOIN
       Restaurante ON Restaurante.idRestaurante == ID
